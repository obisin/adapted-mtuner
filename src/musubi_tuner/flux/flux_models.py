...
class DoubleStreamBlock(nn.Module):
    ...
    def _forward(
        ...
        # Fix: Remove all singleton dimensions from modulation to prevent broadcasting issues
        def squeeze_to_2d(tensor):
            """Remove all singleton dimensions except batch and feature dims"""
        ...
        scale = squeeze_to_2d(img_mod1.scale)
        shift = squeeze_to_2d(img_mod1.shift)
        img_modulated = (1 + scale) * img_modulated + shift
        ...
        txt_scale = squeeze_to_2d(txt_mod1.scale)
        txt_shift = squeeze_to_2d(txt_mod1.shift)
        txt_modulated = (1 + txt_scale) * txt_modulated + txt_shift
        ...
        img_gate1 = squeeze_to_2d(img_mod1.gate)
        img = img + img_gate1 * self.img_attn.proj(img_attn)

        img_scale2 = squeeze_to_2d(img_mod2.scale)
        img_shift2 = squeeze_to_2d(img_mod2.shift)
        img_gate2 = squeeze_to_2d(img_mod2.gate)
        img = img + img_gate2 * self.img_mlp((1 + img_scale2) * self.img_norm2(img) + img_shift2)

        # calculate the txt blocks
        txt_gate1 = squeeze_to_2d(txt_mod1.gate)
        txt = txt + txt_gate1 * self.txt_attn.proj(txt_attn)

        txt_scale2 = squeeze_to_2d(txt_mod2.scale)
        txt_shift2 = squeeze_to_2d(txt_mod2.shift)
        txt_gate2 = squeeze_to_2d(txt_mod2.gate)
        txt = txt + txt_gate2 * self.txt_mlp((1 + txt_scale2) * self.txt_norm2(txt) + txt_shift2)
        return img, txt
...
class SingleStreamBlock(nn.Module):
    ...
    def _forward(self, x: Tensor, vec: Tensor, pe: Tensor, control_lengths: Optional[list[int]] = None) -> Tensor:
        ...
        scale = squeeze_to_2d(mod.scale)
        shift = squeeze_to_2d(mod.shift)
...
class LastLayer(nn.Module):
    ...
    def forward(self, x: Tensor, vec: Tensor) -> Tensor:
        # Apply modulation and ensure proper shape for chunking
        modulation_out = self.adaLN_modulation(vec)

        # Flatten any extra dimensions - we need [batch_size, features]
        if modulation_out.dim() > 2:
            modulation_out = modulation_out.view(modulation_out.shape[0], -1)
        elif modulation_out.dim() == 1:
            modulation_out = modulation_out.unsqueeze(0)  # Add batch dimension if missing
...
class Flux(nn.Module):
    ...
    def enable_block_swap(self, num_blocks: int, device: torch.device, supports_backward: bool):
        ...
        # Better algorithm: swap single blocks first (smaller), then double blocks (larger)
        # This keeps more expensive double blocks on GPU for better performance
        max_single_swap = self.num_single_blocks - 2  # Keep 2 single blocks on GPU
        max_double_swap = self.num_double_blocks - 2  # Keep 2 double blocks on GPU
        max_total_swap = max_single_swap + max_double_swap

        assert num_blocks <= max_total_swap, (
            f"Cannot swap more than {max_total_swap} blocks total "
            f"(max {max_single_swap} single + {max_double_swap} double blocks). "
            f"Requested {num_blocks} blocks."
        )

        # Swap single blocks first, then double blocks with remainder
    ...
    def move_to_device_except_swap_blocks(self, device: torch.device):
        # Assume model is on CPU. Move only resident blocks to device to reduce peak memory.
        # Hide only the block lists that actually have swaps; resident lists are moved by .to(device).
        save_double_blocks = self.double_blocks
        save_single_blocks = self.single_blocks

        hide_double = (
            self.offloader_double is not None
            and getattr(self.offloader_double, "blocks_to_swap", 0) > 0
        )
        hide_single = (
            self.offloader_single is not None
            and getattr(self.offloader_single, "blocks_to_swap", 0) > 0
        )
        ...
        # Restore references so the offloaders can manage swaps and residency.
